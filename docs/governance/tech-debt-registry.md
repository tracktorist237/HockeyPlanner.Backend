...
|---|---|---|---|---|---|---|
| SEC-001 | Trust in client-supplied `currentUserId` in team operations | P0 | User impersonation | M7 | `HockeyPlanner.Backend.WebAPI/Controllers/TeamsController.cs` (`CreateTeam`, `UpdateTeam`, `UpdateTeamMember`, `RemoveTeamMember`, `JoinByCode`, `JoinPublic`, `LeaveTeam` and team news/media actor-query endpoints) | Open |
| SEC-002 | Public Users CRUD and entity mass assignment | P0 | Hash exposure and role escalation | M3; resolved by `23c50de` and `1af517f` | `HockeyPlanner.Backend.Application/Implementations/Services/UserService.cs` (`GetDirectory`, `UpdateUser`); `HockeyPlanner.Backend.WebAPI/Controllers/UsersController.cs` (`PostUser`, `DeleteUser`, `UploadAvatar`); `tests/HockeyPlanner.Backend.IntegrationTests/Security/UserApiSecurityExpectationTests.cs` (10 active and passing user security expectations) | Resolved |
| SEC-003 | Cross-team event update and unprotected event/roster reads | P0 | IDOR and private-data disclosure | M2; resolved by `ce6dc25` and `3b86556` | `HockeyPlanner.Backend.Application/Implementations/Services/EventService.cs` (`UpdateEvent`, `GetEvent`); `HockeyPlanner.Backend.Application/Implementations/Services/LineService.cs` (`GetRosterByEvent`); `tests/HockeyPlanner.Backend.IntegrationTests/Security/EventAuthorizationSecurityExpectationTests.cs` (`CrossTeamUpdate_IsForbidden_AndForeignEventRemainsUnchanged`, `PrivateEventRead_ByUserFromAnotherTeam_IsForbidden`, `PrivateRosterRead_ByUserFromAnotherTeam_IsForbidden`); full integration suite: 74 passed, 0 skipped, 0 failed | Resolved |
| SEC-004 | Arbitrary player deletion by identifier | P0 | Roster corruption | M2; resolved by `3b86556` | `HockeyPlanner.Backend.Application/Implementations/Services/PlayerService.cs` (`RemovePlayerById`, resource graph `Player -> Line -> Event -> Team -> TeamMembership`); `tests/HockeyPlanner.Backend.IntegrationTests/Security/AttendanceAndPlayerSecurityExpectationTests.cs` (`DeleteForeignPlayer_IsForbidden_AndPlayerRemainsInRoster`); full integration suite: 74 passed, 0 skipped, 0 failed | Resolved |
...
| SEC-007 | Trust in client-supplied `currentUserId` in notification operations | P0 | Notification disclosure and impersonation | M3; resolved by `d67f71e` | `HockeyPlanner.Backend.Application/Implementations/Services/UserNotificationService.cs` (JWT-owned inbox, mark-read, mark-all, preferences and self-test operations); `tests/HockeyPlanner.Backend.IntegrationTests/Security/NotificationOwnershipSecurityTests.cs` (28 focused ownership tests passing) | Resolved |
...
| ARC-003 | EF entities are API contracts | P1 | Data exposure and fragile contracts | M3, M12; M3 Users/Notifications security-sensitive paths now use DTO and service boundaries; complete API-wide enforcement remains M12 | `HockeyPlanner.Backend.WebAPI/Controllers/UsersController.cs`; `HockeyPlanner.Backend.WebAPI/Controllers/NotificationsController.cs`; M12 architecture and contract audit remains outstanding | Open |
...
| PERF-003 | Users/events/releases and birthday scans can be unbounded | P3 | Future memory and latency growth | Deferred; revisit in M10-M12 | `HockeyPlanner.Backend.Application/Implementations/Services/UserService.cs` (`GetDirectory`); `HockeyPlanner.Backend.WebAPI/Services/BirthdayPushHostedService.cs` (`ExecuteAsync`) | Accepted |
