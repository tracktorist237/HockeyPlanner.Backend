...
| SEC-007 | Trust in client-supplied `currentUserId` in notification operations | P0 | Notification disclosure and impersonation | M3 | `HockeyPlanner.Backend.WebAPI/Controllers/NotificationsController.cs` (`GetNotifications`, `MarkRead`, `MarkAllRead`, `GetPreferences`, `UpdatePreferences`, `SendTest`) | Open |
